...
def send(message):
    ...
    respond(message)
def respond(message):
    intents =match_intent(message)
    if intents in responses:
        key = intents
        print(bot_template.format(responses[key]))
    if intents=='weather':
        get_now_weather = get_weather('南京')
        print(bot_template.format(get_now_weather))
    if intents=='team':
        b = get_rank()
        print(bot_template.format(b))
    if intents=='result':
        a = get_nba()
        print(bot_template.format(a))
send('hello')
